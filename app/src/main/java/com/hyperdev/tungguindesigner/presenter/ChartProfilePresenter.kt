...
class ChartProfilePresenter(private val view: ChartOrderView.View,
                            private val chart: ChartRepositoryImpl,
                            private val scheduler: SchedulerProvider) : ChartOrderView.Presenter{
    ...
    override fun getChartItem(token: String, context: Context) {
        ...
        compositeDisposable.add(chart.getChartOrder(token, "application/json")
            ...
            .subscribeWith(object : ResourceSubscriber<ChartResponse>(){
                ...
                override fun onNext(t: ChartResponse) {
                    try{
                        t.data?.let { it.item?.let { it1 -> view.displayChart(it1) } }
